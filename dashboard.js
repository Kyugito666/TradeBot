...
function updateLeverageLimits() {
  ...
  let maxLev = 50;
  if (exchange === 'mexc') {
    maxLev = (symbol.includes('BTC') || symbol.includes('ETH')) ? 200 : 100;
...
function updatePriceStats() {
  ...
  if (stats.prevPrice && stats.prevPrice !== p) {
    ...
    const cls = 'stat-value ' + (up ? 'up' : 'down');
...
function updateLSRStats() {
  if (!stats.lsr) return;
  const lsrEl = document.getElementById('stat-lsr');
  const biasEl = document.getElementById('stat-bias');
  if (lsrEl) lsrEl.textContent = stats.lsr.toFixed(4);
  if (biasEl) {
    biasEl.textContent = stats.bias.split('_')[0];
    biasEl.className = 'bias-badge ' + stats.bias;
  }
}
...
async function initChart() {
  ...
  if (!loaded) {
    ...
    if (el) el.textContent = '⚠ lib load failed';
    return;
  }
...
async function fetchOHLCV(tf) {
  ...
  try {
    ...
    const candles = rows.slice().reverse().map(r => ({
      time: Math.floor(parseInt(r[0]) / 1000),
      open: parseFloat(r[1]), high: parseFloat(r[2]),
      low:  parseFloat(r[3]), close: parseFloat(r[4]),
      volume: parseFloat(r[5])
    }));
    _ohlcvCache[intv] = candles;
    _lastCandleTs = candles[candles.length - 1]?.time || 0;
    if (statusEl) { statusEl.textContent = `✓ ${sym} ${_tfLabel(intv)}`; statusEl.className = 'chart-status ok'; }
    return candles;
  } catch(e) {
    if (statusEl) { statusEl.textContent = `✗ ${e.message}`; statusEl.className = 'chart-status err'; }
    return null;
  }
}

function _tfLabel(tf) {
  return {'1':'1m','3':'3m','5':'5m','15':'15m','30':'30m','60':'1h','120':'2h','240':'4h','D':'1d'}[tf] || tf;
}
...
function renderTradesTableGo(active, history) {
  ...
  if (active && active.length > 0) {
    ...
    html += `<tr style="background:rgba(30,41,59,0.5)">
      <td>${p.time || '--'}</td><td class="${cls}">${p.side} (LIVE)</td>
      <td>${(p.entry_price||0).toFixed(4)}</td><td class="trade-win">${(p.take_profit||0).toFixed(4)}</td>
      <td class="trade-loss">${(p.stop_loss||0).toFixed(4)}</td>
      <td style="color:var(--accent)">OPEN</td>
      <td class="${pnlColor}" style="font-weight:bold">${pnlSign}${(p.pnl||0).toFixed(2)}%</td></tr>`;
  }
  if (history && history.length > 0) {
    history.slice(0, 20).forEach(p => {
      ...
      const pnlColor = (p.pnl || 0) >= 0 ? 'trade-win' : 'trade-loss';
      const pnlSign = (p.pnl || 0) >= 0 ? '+' : '';
      html += `<tr><td>${p.time||'--'}</td><td class="${cls}">${p.side}</td>
        <td>${(p.entry_price||0).toFixed(4)}</td><td>${(p.take_profit||0).toFixed(4)}</td>
        <td>${(p.stop_loss||0).toFixed(4)}</td><td class="${cls}">${p.status||'--'}</td>
        <td class="${pnlColor}">${pnlSign}${(p.pnl||0).toFixed(2)}%</td></tr>`;
    });
  }
  if (html === '') {
...
function renderTradesTable() {
  ...
  if (activeTrade) {
    ...
    rows += `<tr>
      <td>${activeTrade.time}</td><td class="trade-open">${activeTrade.action}</td>
      <td>${activeTrade.entry.toFixed(dec)}</td>
      <td class="trade-win">${activeTrade.tp.toFixed(dec)}</td>
      <td class="trade-loss">${activeTrade.sl.toFixed(dec)}</td>
      <td class="trade-open">OPEN</td><td class="trade-open">--</td></tr>`;
  }
  tradeHistory.slice(0, 20).forEach(t => {
    const cls = t.result === 'WIN' ? 'trade-win' : 'trade-loss';
    const dec = t.entry < 1 ? 6 : t.entry < 10 ? 4 : 4;
    const pnl = (t.pnl_pct >= 0 ? '+' : '') + t.pnl_pct.toFixed(2) + '%';
    rows += `<tr>
      <td>${t.time}</td><td class="${cls}">${t.action}</td>
      <td>${t.entry.toFixed(dec)}</td><td>${t.tp.toFixed(dec)}</td>
      <td>${t.sl.toFixed(dec)}</td><td class="${cls}">${t.result}</td>
...
function renderLogs() {
  ...
  if (!filtered.length) {
    container.innerHTML = '<div class="empty-state"><div class="empty-icon">◈</div><div>Tidak ada log untuk filter ini</div></div>';
    return;
  }
...
async function loadBybitPairs(exchange) {
  const statusEl = document.getElementById('pairs-status');
  const sel = document.getElementById('cfg-symbol');
  ...
  try {
    ...
    const res = await fetch(`${_BYBIT_INSTRUMENTS_URL}?${params}`);
    ...
    const hdrSym = document.getElementById('hdr-symbol');
    if (hdrSym) hdrSym.textContent = sel.value;
    sel.onchange = () => { _onSymbolChange(sel.value); };
  } catch(err) {
    statusEl.textContent = '✗ fetch failed'; statusEl.style.color = 'var(--danger)';
  }
}
...
function _onSymbolChange(sym) {
  ...
  if (hdrPrice) hdrPrice.textContent = '--';
  if (statPrice) statPrice.textContent = '--';
}

function onExchangeChange(val) { loadBybitPairs(val); updateLeverageLimits(); saveConfig(); }

let _wsTicker = null;
function _getLiveSymbol() {
  const sel = document.getElementById('cfg-symbol');
